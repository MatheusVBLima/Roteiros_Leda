...
public class QuickSelect<T extends Comparable<T>> {
	...
	public T quickSelect(T[] array, int k) {
		return sortAux(array, k, 0, array.length);
	}

	public T sortAux(T[] array, int k, int leftIndex, int rightIndex) {
		...
		if (leftIndex < rightIndex) {
			...
			if (posicaoPivot > k - 1) {
				return sortAux(array, k, leftIndex, posicaoPivot - 1);
			} else if (posicaoPivot < k - 1) {
				return sortAux(array, k, posicaoPivot + 1, rightIndex);
			}
		}
	...
	private int separar(T[] array, int leftIndex, int rightIndex) {
		...
		for (int j = leftIndex + 1; j <= rightIndex; j++) {
			if (array[j].compareTo(array[pivot]) <= 0) {
				chave += 1;
				Util.swap(array, chave, j);
			}

		}
		Util.swap(array, pivot, chave);
		return chave;
	}
}
